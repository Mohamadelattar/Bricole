...
trier()
{
     for (let i = 0; i < this.messageUsers.length; i++) {
        ...
        if(Number(this.messageUsers[i].freelancer_id) === Number(this.messageUsers[index].freelancer_id))
          { 
...
getUsers()
{
  ...
  for (let i = 0; i < this.messageUsers.length; i++) {
    this.num = Number(this.messageUsers[i].freelancer_id);
    this.freelanceService.getFreelancer(Number(this.messageUsers[i].freelancer_id)).subscribe(res =>{
    this.freelancer = res;
    this.users.push(res);
